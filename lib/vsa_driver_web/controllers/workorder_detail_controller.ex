defmodule VsaDriverWeb.WorkorderDetailController do
  ...
  def create(conn, %{"workorder_detail" => workorder_detail_params}) do
    with {:ok, %WorkorderDetail{} = workorder_detail} <- Core.create_workorder_detail(workorder_detail_params) do
      conn
      |> put_status(:created)
  ...
  def update(conn, %{"id" => id, "workorder_detail" => workorder_detail_params}) do
    ...
    with {:ok, %WorkorderDetail{} = workorder_detail} <- Core.update_workorder_detail(workorder_detail, workorder_detail_params) do
      render(conn, "show.json", workorder_detail: workorder_detail)
    end
  end
